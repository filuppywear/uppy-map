...
export default function ProfileView({ onAuthRequired }: { onAuthRequired: () => void }) {
  ...
  useEffect(() => {
    ...
    getProfile()
      .then((p) => {
        if (p) {
          setProfile(p);
          setForm({ username: p.username ?? "", full_name: p.full_name ?? "", phone: p.phone ?? "", bio: p.bio ?? "" });
        }
      })
      .catch((err) => { console.error("Profile load failed:", err); setLoadError("Failed to load profile"); });
  }, [isLoggedIn]);
    ...
    <div className="h-full overflow-y-auto" style={{ background: "#2D2323" }}>
      <div className="max-w-lg mx-auto px-4 md:px-6 py-6 md:pt-8 pb-28 lg:pb-10">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="w-16 h-16 flex items-center justify-center" style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: "50%" }}>
        ...
        {!editing ? (
          <div className="flex flex-col gap-4">
            {profile?.bio && <p className="text-sm" style={{ color: "rgba(255,255,255,0.6)" }}>{profile.bio}</p>}
            {profile?.phone && <p className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>Phone: {profile.phone}</p>}
            <button onClick={() => setEditing(true)} className="self-start px-4 py-2 text-xs font-bold uppercase tracking-[0.1em]" style={{ border: "1px solid rgba(255,255,255,0.2)", color: "rgba(255,255,255,0.5)", background: "transparent", cursor: "pointer" }}>
              Edit profile
            </button>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            ...
            {saveError && <p className="text-xs mt-2" style={{ color: "#E4A08A" }}>{saveError}</p>}
          </div>
        )}

        <div className="mt-10 flex flex-col gap-3">
          <Link href="/favorites" className="flex items-center gap-2 font-bold uppercase" style={{ fontSize: "11px", letterSpacing: "0.1em", color: "rgba(255,255,255,0.4)", textDecoration: "none" }}>
          ...
          <Link href="/proposals" className="flex items-center gap-2 font-bold uppercase" style={{ fontSize: "11px", letterSpacing: "0.1em", color: "rgba(255,255,255,0.4)", textDecoration: "none" }}>
        ...
        <div className="mt-8 pt-6" style={{ borderTop: "1px solid rgba(255,255,255,0.1)" }}>
          <button onClick={signOut} className="font-bold uppercase px-4 py-2" style={{ fontSize: "10px", letterSpacing: "0.1em", border: "1px solid rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.3)", background: "transparent", cursor: "pointer" }}>
            Sign out
          </button>
        </div>
      </div>
